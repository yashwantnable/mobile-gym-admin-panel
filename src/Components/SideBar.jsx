    ...
    <>
      <div className='w-64 min-h-screen bg-primary text-white flex flex-col'>
        ...
        <nav className='flex-1 mt-4'>
          {(user?.user_role?.name === 'trainer' ? TrainerMEnuItems : menuItems).map(
            ({ name, icon: Icon, path, submenu }) => (
              <div key={name}>
                <NavLink
                  to={path}
                  className={`flex items-center justify-between space-x-3 px-6 py-3 text-sm font-medium transition-colors ${
                    active === path || (submenu && submenu.some((item) => active === item.path))
                      ? 'bg-secondary'
                      : 'hover:bg-[#afc2d5]'
                  }`}
                  onClick={(e) => {
                ...
                {submenu && (
                  <div
                    className={`ml-8 overflow-hidden pt-2 transition-all duration-300 ${
                      openSubmenu === name ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
                    } bg-primary`}
                  >
                    {submenu.map((item) => (
                      <NavLink
                        ...
                        className={`flex items-center space-x-3 px-6 py-2 text-sm font-medium transition-colors ${
                          active === item.path ? 'bg-secondary text-black' : 'hover:bg-[#afc2d5]'
                        }`}
